...
#define EDGE(a, b)                                                             \
    ...
    for (mc_ind_t q = 1; q < contour_count; ++q) {
        ...
        float const cos = cosf(theta) * norm;
        float const sin = sinf(theta) * norm;
        struct vec3 const delta = vec3_add(
            vec3_mul_scalar(cos, plane.a), vec3_mul_scalar(sin, plane.b)
        );

        for (mc_ind_t k = 0; k < sub_counts[q]; ++k) {
            vertices[q][3 * k] += delta.x;
            vertices[q][3 * k + 1] += delta.y;
            vertices[q][3 * k + 2] += delta.z;
        }
    }
