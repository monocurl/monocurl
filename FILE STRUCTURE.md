...
- executor: takes bytecode and scene state and executes it
  ...
  - src/executor/ops.rs: binary/unary operations with int→float→complex type promotion plus recursive list linear ops (scalar multiply, elementwise add, negate); owns BinOp enum (pub(crate)); intercepts Value::Stateful operands to build StatefulNode::BinaryOp/UnaryNeg/Not lifted nodes instead of evaluating eagerly
  ...
  - src/value/leader.rs: Leader struct (kind: LeaderKind, leader_rc, follower_rc as RcValues, detached status via last_modified_stack, active animation lock, follower_version u64 for stateful cache invalidation) for mesh/param variables
  ...
  - src/value/stateful.rs: Stateful (roots: Vec<RcValue> + root: StatefulNode + version-based cache) and StatefulNode enum for reactive dependency graphs; variants: LeaderRef, Constant, LabeledCall, LabeledOperatorCall, BinaryOp, UnaryNeg, Not, Subscript; LabeledCall/LabeledOperatorCall args are Vec<RcValue> enabling mutable attribute access; async Executor::eval_stateful evaluates the full node tree with follower-version caching
...
- ui: contains all ui related code (everything is gpui)
  ...
  - src/services/{mod.rs,lexing.rs,compilation.rs,execution.rs}: background services that run off the main thread. mod.rs contains ServiceManager and ServiceManagerMessage (including ExecutionSnapshot re-export). execution.rs spawns a dedicated OS thread that owns the Executor (Rc-based, !Send), handles UpdateBytecode/SeekTo/TogglePlay/UpdateParameter, formats runtime diagnostics with recovered callstacks and root line numbers, and sends ExecutionSnapshot back via ServiceManagerMessage::ExecutionStateUpdated, including current parameter values plus temporary mesh leader/follower debug strings for presentation mode
  - src/state: code related to global, document, text editor, and similar state
    ...
    - execution_state.rs: UI-side execution state (current_timestamp, runtime_errors, slide_durations, executor-reported minimum duration hints, current parameter snapshot, temporary mesh debug snapshot, slide_count, is_playing) updated via ExecutionSnapshot from the executor thread
  ...
  - src/viewport: a module related to viewport rendering; presentation mode currently shows parameter sliders plus temporary mesh leader/follower debug blocks
