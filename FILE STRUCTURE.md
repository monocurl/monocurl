...
- executor: takes bytecode and scene state and executes it
  ...
  - src/executor/runtime_error.rs: recovered runtime error span/callstack construction from stack ancestry and saved call frames, including play-aware recovery and outermost-first displayed callstacks so spawning `play` sites appear before inner imported helper frames; plus public helpers to record an `ExecutorError` either at the current execution context or explicitly at the root stack when a subsystem such as UI scene snapshotting needs the diagnostic to anchor near the current top-level playback position. Uses best-effort annotation fallback so explicitly recorded errors still land on a visible source span when no precise instruction span is available
  ...
  - src/scene_snapshot.rs: shared background/camera/visible-mesh capture for any frontend. Validates scene leaders by eliding live function/operator wrappers, parses camera/background values into concrete snapshot structs, collects visible mesh trees, and can record runtime errors at the root playback position when scene capture fails
