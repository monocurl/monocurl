...
- executor: takes bytecode and scene state and executes it
  ...
  - src/scene_snapshot.rs: shared background/camera/visible-mesh capture for any frontend. Validates scene leaders by eliding live function/operator wrappers, parses camera/background values into concrete snapshot structs, collects visible mesh trees, and returns a `Result` from stable snapshot capture so callers can see directly whether scene capture recorded a root-position runtime error
