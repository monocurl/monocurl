...
// collects only &name-style lvalue references that are free (not locally defined)
struct LvalueRefCollector {
    defined: HashSet<String>,
    lvalue_refs: Vec<String>,
    seen: HashSet<String>,
}
...
impl LvalueRefCollector {
    ...
    fn visit_expr(&mut self, expr: &Expression) {
        match expr {
            Expression::IdentifierReference(IdentifierReference::Reference(n)) => self.add(n),
            ...
            Expression::LambdaDefinition(l) => {
                ...
                // propagate lvalue refs from inner body, scoped by inner params
                let mut inner_pre: HashSet<String> =
                    l.args.iter().map(|a| a.identifier.1.0.clone()).collect();
                if matches!(l.body.1, LambdaBody::Block(_)) {
                    inner_pre.insert("_".to_string());
                }
                let mut inner = LvalueRefCollector::new(inner_pre);
                match &l.body.1 {
                    LambdaBody::Inline(e) => inner.visit_expr(e),
                    LambdaBody::Block(s) => inner.visit_stmts(s),
                }
                for name in inner.lvalue_refs {
                    self.add(&name);
                }
            }
            Expression::OperationDefinition(o) => self.visit_expr(&o.lambda.1),
            Expression::Block(b) => {
                let mut inner = LvalueRefCollector::new(HashSet::from(["_".to_string()]));
                inner.visit_stmts(&b.body);
                for name in inner.lvalue_refs {
                    self.add(&name);
                }
            ...
            Expression::Anim(a) => {
                let mut inner = LvalueRefCollector::new(HashSet::new());
                inner.visit_stmts(&a.body);
                for name in inner.lvalue_refs {
                    self.add(&name);
                }
...
pub(super) fn free_lvalue_refs_expr(expr: &Expression, pre: HashSet<String>) -> Vec<String> {
    let mut c = LvalueRefCollector::new(pre);
    c.visit_expr(expr);
    c.lvalue_refs
}
