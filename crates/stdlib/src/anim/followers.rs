...
use super::helpers::{
    ...
    read_time, rebuild_mesh_value_like, rebuild_mesh_value_like_by_tag, resolve_targets,
};
...
pub async fn trans_embed(executor: &mut Executor, stack_idx: usize) -> Result<Value, ExecutorError> {
    ...
    let aligned = rebuild_mesh_value_like(&start_leaves, &destination)?;
    Ok(embed_triplet(aligned, destination, Value::Nil))
}
...
pub async fn tag_trans_embed(
    ...
    let aligned = rebuild_mesh_value_like_by_tag(&by_tag, &start_leaves, &destination)?;
    Ok(embed_triplet(aligned, destination, Value::Nil))
}
...
pub async fn trans_lerp_value(
    ...
    mesh_tree_patharc_lerp(&start, &end, t, path_arc)
}
