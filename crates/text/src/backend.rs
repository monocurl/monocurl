...
mod browser {
    ...
    #[wasm_bindgen]
    extern "C" {
        #[wasm_bindgen(js_namespace = globalThis, js_name = __monocurlRenderLatexSvg, catch)]
        fn monocurl_render_latex_svg(kind: &str, source: &str) -> Result<JsValue, JsValue>;
    }

    pub(super) fn render_svg(kind: BackendKind, source: &str) -> Result<String> {
        let rendered = monocurl_render_latex_svg(kind.as_str(), source)
    ...
    fn js_error_message(value: JsValue) -> String {
        value
            .as_string()
            .unwrap_or_else(|| "browser text backend failed".into())
    }
}
